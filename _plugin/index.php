...
$_core -> Call('Actions', 'Add', array('plugins_loaded', function () use ($_plugin, $_core){
	...
	if(isset($sections_prefixes[$current_section_id])){
		...
		$head_enabled = $_plugin -> OptionGet($section_prefix.'_css_head_enabled');
		$foot_enabled = $_plugin -> OptionGet($section_prefix.'_css_foot_enabled');
		
		if(($head_enabled) || ($foot_enabled)){
			
			if($head_enabled){
				
				$head_code_safe = $_plugin -> OptionGet($section_prefix.'_css_head_code_safe');
			...
			if($foot_enabled){
				...
				$foot_code_safe = $_plugin -> OptionGet($section_prefix.'_css_foot_code_safe');
